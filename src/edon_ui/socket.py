...
# --- SocketCircleItem (formerly from socket_circle_item.py) --- #
class SocketCircleItem(QGraphicsObject):
    ...
    def __init__(self, parent, is_input: bool, identifier: any, visual_type_key: str = "default"):
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            print(f"SocketCircleItem {self.identifier} pressed at {event.scenePos()}")
            if self.scene(): # Ensure scene exists
    ...
    def mouseMoveEvent(self, event):
        if self.scene() and self.scene().is_dragging_edge():
    ...
    def mouseReleaseEvent(self, event):
        if event.button() == Qt.LeftButton:
            print(f"SocketCircleItem {self.identifier} released at {event.scenePos()}")
            if self.scene() and self.scene().is_dragging_edge():
...
class SocketRowItem(QGraphicsObject):
    ...
    def __init__(
        self, parent, is_input: bool, socket_identifier: any, socket_visual_type: str = "default", label_text: str = ""
    ):
        super().__init__(parent)
        self.is_input = is_input
        # label_text is not used for the placeholder, but kept for future compatibility
        ...
        self.socket_circle = SocketCircleItem(
            self, is_input=self.is_input, identifier=socket_identifier, visual_type_key=socket_visual_type
        )
