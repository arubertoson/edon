...
class TestSubgraphNavigation:
    def test_full_enter_and_exit_subgraph_workflow(
        ...
        assert workspace_controller.graph_context_stack.is_at_root()
        assert workspace_controller.graph_context_stack.depth == 1
        original_scene = workspace_controller.scene
        original_graph = workspace_controller.graph
        assert original_graph is root_graph # After load_graph, controller's graph should be the new root graph instance
        assert len(original_scene.items()) > 0 # Nodes should be populated

        # Get the NodeItem for the SubGraphNode
        subgraph_node_item = workspace_controller.data.node_item_for_id(
        ...
        assert not workspace_controller.graph_context_stack.is_at_root()
        assert workspace_controller.graph_context_stack.depth == 2
        
        current_nav_state = workspace_controller.graph_context_stack.current_level
        ...
        assert current_nav_state.originating_subgraph_node is subgraph_node_entity

        # Check if the new scene is populated with the internal graph's contents
        # (e.g., simple_internal_node should be there)
        internal_node_ui_item = workspace_controller.data.node_item_for_id(
        ...
        assert workspace_controller.graph_context_stack.is_at_root()
        assert workspace_controller.graph_context_stack.depth == 1
        assert workspace_controller.graph is original_graph # Should be back to the root graph instance
        assert workspace_controller.scene is original_scene # Should be back to the root scene instance
        
        # Verify root scene content is back
        root_subgraph_node_item_after_exit = workspace_controller.data.node_item_for_id(
            subgraph_node_entity.id
        )
    ...
    def test_enter_subgraph_conditions(
        ...
        initial_depth = workspace_controller.graph_context_stack.depth
        selected_node_items: list[NodeItem] = []

        subgraph_node_item = workspace_controller.data.node_item_for_id(subgraph_node_entity.id)
        regular_node_item = workspace_controller.data.node_item_for_id(regular_node_entity.id)
        ...
        assert success is False, f"Expected action_enter_subgraph to fail for config: {selected_items_config}"
        assert workspace_controller.graph_context_stack.depth == initial_depth
